...
fn render_layout_body(lines: &[&str], source_len: usize, options: &Options<'_>) -> Option<String> {
    ...
    while i < lines.len() {
        ...
        if line.starts_with("- ") {
            i = render_layout_list(lines, i, 0, depth, options, &mut out)?;
            wrote = true;
            continue;
        }
        ...
        if line.starts_with('|') {
            i = render_layout_table(lines, i, depth, options, &mut out)?;
            wrote = true;
            continue;
        }
    ...
    while i < lines.len() {
        ...
        if i < lines.len() {
            ...
            if next_indent > indent {
                ...
                out.push('\n');
                i = render_layout_list(lines, i, indent + 2, depth + 2, options, out)?;
                out.push('\n');
                layout_indent(out, depth + 1);
            }
        }
