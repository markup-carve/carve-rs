...
/// Put a STRUCTURAL link/image/reference title into its namespaced wire slot.
///
/// `title` remains the authored `{title=...}` attribute, so a node carrying
/// both spellings loses neither and its `carveAttrOrder` remains truthful
/// (carve-rs#1115).
fn set_link_title(a: &mut Object, title: &str) {
    a.insert("carveLinkTitle".into(), Json::String(title.to_owned()));
}
