...
/// The canonical container fence: three colons outermost, one wider per level.
///
/// Since carve#439 a closer must match its opener EXACTLY, so a fence no
/// longer has to outrank anything - it pairs like a tag. That removed the
/// reason to size a fence from a container's DESCENDANTS (it had to be wider
/// than everything nested inside it, so adding a level meant rewriting the
/// whole chain) and lets the width come from the container's own depth.
///
/// A container inside a blockquote, a list item or a definition body starts
/// over at three colons: its fence lines carry that host's prefix or indent,
/// so they cannot pair with an ancestor's fence.
fn colon_fence_for(ctx: &CarveContext) -> String {
    ":".repeat(3 + ctx.colon_depth)
}
