...
- `TableRowGroups` adds `head_attrs` and `foot_attrs` as `Option<Box<Attrs>>`. Downstream struct literals must initialize both fields to `None` when unused. Package releases remain on the 0.1 patch line.
