...
fn no_resolver_leaves_include_directives_literal_without_warnings() {
    ...
    assert_eq!(
        render_html(&result.doc).unwrap(),
        "<p>See {{ child.crv }} here.</p>"
    );
}
...
mod rules {
    ...
    fn i3_resolver_model_no_resolver_attempts_no_resolution_at_all() {
        ...
        assert!(result.warnings.is_empty());
        assert!(result.dependencies.is_empty());
        assert_eq!(render_html(&result.doc).unwrap(), "<p>See {{ child }}.</p>");
    }
    ...
    fn i1_an_empty_or_pathless_token_is_not_a_directive_and_never_reaches_the_resolver() {
        for source in [
            ...
            assert_eq!(render_html(&result.doc).unwrap(), literal_html(source));
        }
    }
...
mod rejected_directives_have_no_side_effects {
    ...
    fn size_limit_has_no_side_effects() {
        ...
        // THE BUDGET IS THE ONE REJECTION THAT IS TERMINAL, so the release is
        // not observable through a LATER include the way it is for a cycle or a
        // depth refusal. Section 19 requires that once the budget is spent every
        // remaining directive degrades to literal WITHOUT being resolved - the
        // refusal is already decided, and resolving to reach it is the read
        // amplification the budget exists to prevent. The spec's own security
        // vector `budget-exhaustion-skips-later-resolver` pins it.
        //
        // So the property asserted here is that terminal behavior. The
        // reservation release itself is still proven by the non-terminal
        // rejections above.
        let later = tuned(
            "{{ big }}\n\n{{ good }}",
        ...
        assert!(
            !later.html.contains("id=\"dup\""),
            "a directive after budget exhaustion was still expanded: {}",
            later.html
        );
        assert!(
            later
                .rules()
                .iter()
                .filter(|r| **r == "include-budget")
                .count()
                >= 2,
            "the directive after exhaustion should report the budget too: {:?}",
            later.rules()
        );
    }
